...
ZERO = Decimal(0)
ONE = Decimal(1)
MSG_NO_PREIS_EINTRITT = (
    "Preis-Eintritt fehlt. Bitte setzen, bevor Preise geschrieben werden."
)
ZURICH = ZoneInfo("Europe/Zurich")


def default_preis_eintritt() -> datetime:
    today = datetime.now(ZURICH).date()
    return datetime.combine(today, time.min, tzinfo=ZURICH)


def parse_preis_eintritt(raw: str) -> datetime | None:
    text = (raw or "").strip()
    if not text:
        return None
    try:
    ...
    return datetime.combine(day, time.min, tzinfo=ZURICH)


def format_preis_eintritt_input(value: datetime | None) -> str:
    if value is None:
        return ""
    if value.tzinfo is None:
        value = value.replace(tzinfo=UTC)
    return value.astimezone(ZURICH).date().isoformat()
...
def create_pull_run(
    ...
    *,
    ...
    run = SupplySourceRun(
        ...
        preis_eintritt=default_preis_eintritt(),
...
def create_upload_run(
    ...
    *,
    ...
    upload = SupplySourceUpload(
        ...
        parse_summary={
            "accepted": sum(1 for r in parsed.rows if r.listenpreis is not None),
            "rejected": parsed.row_errors,
            "priceless": parsed.priceless,
    ...
    for item in parsed.rows:
        db.add(
            SupplySourceRow(
                ...
                listenpreis_raw=item.listenpreis_raw,
                rabattcode=item.rabattcode,
                unit_id=item.unit_id,
                unit_raw=item.unit_raw,
                file_unit_id=item.file_unit_id,
...
def retry_failed_run(
    ...
    *,
    ...
    if run.status != "failed":
        raise SupplySourceRunError("Nur fehlgeschlagene Läufe können erneut abgeglichen werden.")
...
def _is_priceless(row: SupplySourceRow) -> bool:
    return row.listenpreis is None


def summary_counts(rows: list[SupplySourceRow]) -> dict[str, Any]:
    ...
    def _count(intent: str) -> int:
        return sum(
            ...
            for r in rows
            if r.included
            and r.row_intent == intent
            and not _is_priceless(r)
        )

    return {
        "update": _count("update"),
        "price_only": _count("price_only"),
        "create": _count("create"),
        "renumber": _count("renumber"),
        "unmatched": sum(
            1
            for r in rows
        ...
        "priceless": sum(
            1
            for r in rows
            if r.included and r.row_intent != "skip" and _is_priceless(r)
        ),
        "skip": sum(1 for r in rows if r.row_intent == "skip" or not r.included),
        "total": len(rows),
        "attach": _count("attach"),
        "missing_unit_raws": missing_unit_raws(rows),
    }


def approval_blockers(rows: list[SupplySourceRow]) -> dict[str, Any]:
    unmatched = 0
    create_no_unit = 0
    attach_no_unit = 0
    priceless_price = 0
    ...
    for row in rows:
        ...
        if row.row_intent == "attach" and not str(row.unit_id or "").strip():
            attach_no_unit += 1
        if _is_priceless(row) and row.row_intent in {"update", "price_only"}:
            priceless_price += 1
    return {
        "unmatched": unmatched,
        "discount_unset": len(sans_unset),
        ...
        "priceless_price": priceless_price,
        "missing_unit_raws": missing_unit_raws(rows),
    }


def missing_unit_raws(rows: list[SupplySourceRow]) -> list[str]:
    words: list[str] = []
    seen: set[str] = set()
    for row in rows:
        if row.row_intent == "skip" or not row.included:
            continue
        if row.row_intent not in {"create", "attach"}:
            continue
        if str(row.unit_id or "").strip():
            continue
        label = (row.unit_raw or "").strip() or "(leer)"
        if label in seen:
            continue
        seen.add(label)
        words.append(label)
...
def apply_edits(
    ...
    for edit in edits:
        ...
        if field in {"rabatt_1", "rabatt_2"}:
            ...
            if row.row_intent not in {"create", "attach"}:
                from app.weclapp_units import UNIT_LOCKED_ERROR

                raise SupplySourceRunError(UNIT_LOCKED_ERROR)
...
def approve_run(db: Session, run: SupplySourceRun, user: Mapping[str, Any]) -> None:
    ...
    if not can_approve(rows):
        raise SupplySourceRunError(
            "Freigabe nicht möglich: offene Artikelzuordnungen, fehlende Rabattsätze oder fehlende Einheit."
        )
    if run.preis_eintritt is None:
        raise SupplySourceRunError(MSG_NO_PREIS_EINTRITT)
...
def row_payload(
    ...
    *,
    ...
    return {
        ...
        "listenpreis_raw": row.listenpreis_raw or "",
        ...
        "file_unit_id": row.file_unit_id or "",
        "unit_overwritten": bool(row.unit_overwritten),
        "live_currency_id": row.live_currency_id or "",
        "resolved_currency_id": row.resolved_currency_id or "",
        "currency_overwritten": bool(row.currency_overwritten),
...
CURRENCY_OVERWRITE_HINT = (
    "Einkaufswährung des Laufs weicht von der aktuellen Preiszeile ab. "
    "Die Lauf-Währung wird geschrieben."
)


def build_grid_config(run: SupplySourceRun, rows: list[SupplySourceRow]) -> dict[str, Any]:
    from app.weclapp_units import (
        UNIT_LOCKED_ERROR,
        UNIT_LOCKED_HINT,
        UNIT_OVERWRITE_HINT,
        units_for_dropdown,
    ...
    unit_source = [
        {"id": u["id"], "name": u["name"]} for u in units if (u.get("name") or "").strip()
    ]
    ...
    columns = [
        ...
        {"title": "Artikelzuordnung übernehmen", "width": 180, "type": "checkbox"},
        {
            "title": "Einheit",
            "width": 110,
            "type": "dropdown",
        ...
        {"title": "Artikelzuordnung", "width": 180, "readOnly": True},
        {
            "title": "Vorgang",
            "width": 180,
            "type": "dropdown",
            "source": [
                {"id": "update", "name": "aktualisieren"},
                {"id": "price_only", "name": "nur Preisänderung"},
                {"id": "create", "name": "neu anlegen"},
                {"id": "attach", "name": "zuordnen"},
                {"id": "renumber", "name": "neue Lieferantennummer"},
                {"id": "skip", "name": "auslassen"},
            ],
        },
    ]
    override_source = [
        {"id": "weclapp", "name": "weclapp belassen"},
        {"id": "template", "name": "Vorlagenwert"},
    ]
    if show_div:
        fields.extend(
    ...
    unit_unknown_rows: list[int] = []
    unit_overwrite_rows: list[int] = []
    unit_overwrite_titles: list[str] = []
    unit_unknown_titles: list[str] = []
    currency_overwrite_rows: list[int] = []
    currency_overwrite_titles: list[str] = []
    ...
    for i, payload in enumerate(payloads):
        ...
        match_label = (
            "zugeordnet" if payload["match_status"] == "matched" else "ohne Artikelzuordnung"
        )
        ...
        if (
            payload["row_intent"] in {"create", "attach"}
            and not str(payload["unit_id"] or "").strip()
        ):
            unit_unknown_rows.append(i)
            raw = (payload["unit_raw"] or "").strip() or "(leer)"
            unit_unknown_titles.append(f"Datei-Einheit unbekannt: «{raw}»")
        else:
            unit_unknown_titles.append("")
        if payload["unit_overwritten"]:
            unit_overwrite_rows.append(i)
            file_uid = payload["file_unit_id"]
            file_name = unit_name_by_id.get(file_uid) or file_uid or "—"
            mirror_uid = payload["unit_id"]
            mirror_name = unit_name_by_id.get(mirror_uid) or mirror_uid or "—"
            raw = (payload["unit_raw"] or "").strip() or file_name
            unit_overwrite_titles.append(
                f"{UNIT_OVERWRITE_HINT} Datei: «{raw}» ({file_name}). "
                f"weclapp: {mirror_name}."
            )
        else:
            unit_overwrite_titles.append("")
        if payload["currency_overwritten"]:
            currency_overwrite_rows.append(i)
            live_cid = payload["live_currency_id"] or "—"
            resolved_cid = payload["resolved_currency_id"] or (
                (run.einkaufswaehrung or "").strip() or "—"
            )
            currency_overwrite_titles.append(
                f"{CURRENCY_OVERWRITE_HINT} Live: {live_cid}. "
                f"Lauf: {resolved_cid}."
            )
        else:
            currency_overwrite_titles.append("")
    ...
    return {
        ...
        "unitLockedError": UNIT_LOCKED_ERROR,
        "unitUnknownRows": unit_unknown_rows,
        "unitUnknownTitles": unit_unknown_titles,
        "unitOverwriteRows": unit_overwrite_rows,
        "unitOverwriteTitles": unit_overwrite_titles,
        "unitOverwriteHint": UNIT_OVERWRITE_HINT,
        "currencyOverwriteRows": currency_overwrite_rows,
        "currencyOverwriteTitles": currency_overwrite_titles,
        "currencyOverwriteHint": CURRENCY_OVERWRITE_HINT,
        "units": units,
        ...
        "missingUnitRaws": counts["missing_unit_raws"],
