...
def _retire_stale_party_san(
    db: Session,
    ss_rows: list[dict[str, Any]],
    seen_ss_ids: set[str],
) -> None:
    """Drop local SS rows whose weclapp id changed but party+SAN stayed the same.

    The unique key is (supplier_party_id, supplier_article_number). Inserting a
    new weclapp_id for an existing SAN otherwise raises UniqueViolation and the
    job only shows «Abgleich fehlgeschlagen».
    """
    keys = {
        (row["supplier_party_id"], row["supplier_article_number"])
        for row in ss_rows
        if row.get("supplier_party_id") and row.get("supplier_article_number")
    }
    if not keys:
        return
    existing = list(
        db.scalars(
            select(WeclappSupplySource).where(
                WeclappSupplySource.supplier_party_id.in_({party for party, _ in keys}),
                WeclappSupplySource.supplier_article_number.in_(
                    {san for _, san in keys}
                ),
            )
        ).all()
    )
    stale_ids = [
        row.weclapp_id
        for row in existing
        if (row.supplier_party_id, row.supplier_article_number) in keys
        and row.weclapp_id not in seen_ss_ids
    ]
    if not stale_ids:
        return
    db.execute(
        delete(WeclappSupplySourcePrice).where(
            WeclappSupplySourcePrice.supply_source_weclapp_id.in_(stale_ids)
        )
    )
    db.execute(
        delete(WeclappSupplySourceLink).where(
            WeclappSupplySourceLink.supply_source_weclapp_id.in_(stale_ids)
        )
    )
    db.execute(
        delete(WeclappSupplySource).where(
            WeclappSupplySource.weclapp_id.in_(stale_ids)
        )
    )
...
def pull_supply_source_index(
    ...
    *,
    ...
    _retire_stale_party_san(db, ss_rows, seen_ss_ids)
    _insert_on_conflict(
        db,
        WeclappArticle,
        article_rows,
