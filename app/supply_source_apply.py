...
MSG_NO_EINTRITT = MSG_NO_PREIS_EINTRITT
...
def _currency_id(client: Any, code: str) -> str | None:
...
def _price_start_ms(raw: Mapping[str, Any]) -> int | None:
...
def _price_end_ms(raw: Mapping[str, Any]) -> int | None:
    value = raw.get("endDate")
    if value in (None, ""):
        return None
    try:
        return int(value)
    ...
    *,
    ...
    for raw in live.get("articlePrices") or []:
        if not isinstance(raw, dict):
            continue
        p_start = _price_start_ms(raw)
        if p_start is not None and p_start >= start_ms:
            continue
        row = sanitize_price_row(raw, allowed_ids=allowed)
        p_end = _price_end_ms(row)
        if p_end is None or p_end >= start_ms:
            row["endDate"] = end_ms
        rebuilt.append(row)
    rebuilt.append(
        {
            "price": format_weclapp_price(ek),
...
def apply_summary(rows: list[SupplySourceRow]) -> dict[str, int]:
    priced = [
        r
        for r in rows
        if r.row_intent != "skip" and r.listenpreis is not None
    ]
    priceless = [
        r
        for r in rows
        if r.row_intent != "skip" and r.listenpreis is None
    ]
    intents = [r.row_intent for r in priced]
    return {
        "total": len(intents) + len(priceless),
...
def _apply_price_or_update(ctx: _ApplyCtx, *, intent: str) -> str:
    ...
    currency_id = _currency_id(ctx.client, ctx.run.einkaufswaehrung)
    if not currency_id:
        return _finish(ctx, "REJECTED", message=MSG_NO_CURRENCY)
    live_cid = str((current or {}).get("currencyId") or "").strip()
    ctx.row.resolved_currency_id = currency_id
    if live_cid:
        ctx.row.live_currency_id = live_cid
        if live_cid != currency_id:
            ctx.row.currency_overwritten = True
