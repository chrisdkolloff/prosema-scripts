...
from app.weclapp_units import resolve_upload_unit
...
class ParsedUploadRow:
    ...
    name: str | None
    listenpreis: Decimal | None
    listenpreis_raw: str | None
    ean: str | None
    unit_id: str | None
    unit_raw: str | None
    file_unit_id: str | None
    rabattcode: str | None
...
class ParseResult:
    ...
    priceless: list[str] = field(default_factory=list)
    ...
    for offset, raw_row in enumerate(data_rows, start=2):
        ...
        try:
            ...
            if "negativ" in str(exc):
                ...
                continue
            shown = (listen_raw or "").strip() or "(leer)"
            result.priceless.append(
                f"Zeile {excel_row} ({san}): Listenpreis «{shown}» ist kein Preis."
            )
        unit_raw = cells.get("unit") or None
        unit_id = None
        if unit_raw:
            unit_id = resolve_upload_unit(
                db, unit_raw, supplier_id=supplier_id, units=units
            )
            if unit_id is None:
        ...
        result.rows.append(
            ParsedUploadRow(
                ...
                listenpreis_raw=str(listen_raw).strip() or None,
                ean=ean,
                unit_id=unit_id,
                unit_raw=unit_raw,
