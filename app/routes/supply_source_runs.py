...
from app.supply_source_wizard_csv import render_wizard_csv
from app.supply_source_runs import (
    ...
    can_approve,
    create_pull_run,
    format_preis_eintritt_input,
    parse_preis_eintritt,
    create_upload_run,
    format_pct,
    format_swiss_number,
    list_runs,
...
def download_template(
    ...
    try:
        _template, data = generate_template_xlsx_for_user(db, user=user)
    except SupplySourceTemplateError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    filename = "bezugsquellen-vorlage.xlsx"
    return Response(
        content=data,
...
@router.post("/bezugsquellen/neu/{run_id}/erneut")
def retry_run(
    run_id: int,
    db: Session = Depends(get_db),
    user: SessionUser = Depends(require_user),
    ...
    try:
        retry_failed_run(db, run=run, user=user)
    except SupplySourceRunError as exc:
        return RedirectResponse(
...
def run_detail(
    ...
    return request.app.state.templates.TemplateResponse(
        ...
        "supply_source_runs/detail.html",
        {
            "user": user,
            "run": run,
            "preis_eintritt_date": format_preis_eintritt_input(run.preis_eintritt),
            "rows": rows,
            "summary": summary,
            "blockers": blockers,
...
@router.get("/bezugsquellen/neu/{run_id}/weclapp.csv")
def wizard_csv_download(
    run_id: int,
    db: Session = Depends(get_db),
    user: SessionUser = Depends(require_user),
):
    ...
    try:
        payload, filename = render_wizard_csv(db, run, rows)
    except SupplySourceRunError as exc:
        return RedirectResponse(
            url=f"/bezugsquellen/neu/{run.id}?error={quote(str(exc))}",
            status_code=303,
...
def save_settings(
    ...
    try:
        apply_run_settings(
            ...
            preis_eintritt=parse_preis_eintritt(preis_eintritt),
        )
    except SupplySourceRunError as exc:
        return RedirectResponse(
            url=f"/bezugsquellen/neu/{run.id}?error={quote(str(exc))}",
...
def approve(
    ...
    run = _get_run(db, run_id)
    try:
        parsed = parse_preis_eintritt(preis_eintritt)
        if parsed is not None:
            run.preis_eintritt = parsed
            db.flush()
