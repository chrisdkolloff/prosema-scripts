...
# weclapp validation: Number exceeds permitted format (<13 digits>.<5 digits>)
...
def format_weclapp_price(raw: object) -> str:
    """String price with at most 5 decimal digits (weclapp articlePrices.price)."""
    if raw is None or raw == "":
        raise SupplySourcePayloadError("price is required")
    try:
        value = raw if isinstance(raw, Decimal) else Decimal(str(raw))
    except (InvalidOperation, ValueError) as exc:
        raise SupplySourcePayloadError("price is invalid") from exc
    quantized = value.quantize(_PRICE_QUANTUM, rounding=ROUND_HALF_UP)
    text = format(quantized, "f")
    if "." in text:
        text = text.rstrip("0").rstrip(".")
    intpart = text.lstrip("-").split(".", 1)[0]
    if len(intpart) > 13:
        raise SupplySourcePayloadError("price exceeds weclapp digits")
    return text or "0"
    ...
    *,
    ...
    for key, value in row.items():
        ...
        if key == "reductionAdditions" and isinstance(value, list):
            ...
            continue
        if key == "price":
            cleaned["price"] = format_weclapp_price(value)
            continue
        cleaned[key] = value
    return cleaned
