...
def test_index_replaces_ss_when_weclapp_id_changes(db_session):
    now = __import__("datetime").datetime.now(__import__("datetime").UTC)
    db_session.add(
        WeclappSupplySource(
            weclapp_id="old-ss",
            supplier_party_id="4406",
            supplier_number="10000",
            supplier_article_number="TST-ID-CHANGE",
            weclapp_version="1",
            last_seen_at=now,
        )
    )
    ...
    assert db_session.get(WeclappSupplySource, "old-ss") is None
    fresh = db_session.get(WeclappSupplySource, "new-ss")
    assert fresh is not None
    assert fresh.supplier_article_number == "TST-ID-CHANGE"
