...
def test_rebuild_closes_all_covering_prices_and_drops_later_starts():
    from app.supply_source_apply import epoch_ms, rebuild_article_prices

    eintritt = datetime(2026, 9, 7, 15, 27, tzinfo=UTC)
    start_ms = epoch_ms(eintritt)
    live = {
        "articlePrices": [
            {
                "id": "hist",
                ...
                "currencyId": "261",
                "startDate": 1700000000000,
                "endDate": 1780000000000,
            },
            {
                "id": "open-old",
                "price": "48.9",
                "currencyId": "261",
                "startDate": 1780000000001,
            },
            {
                "id": "399700",
                "price": "49.9",
                "currencyId": "261",
                "startDate": start_ms + 3_600_000,
            },
        ]
    }
    rebuilt = rebuild_article_prices(
        live, ek=Decimal("35.93"), currency_id="261", preis_eintritt=eintritt
    )
    ids = [p.get("id") for p in rebuilt if p.get("id")]
    assert "399700" not in ids
    assert "hist" in ids
    assert "open-old" in ids
    open_old = next(p for p in rebuilt if p.get("id") == "open-old")
    assert open_old["endDate"] == start_ms - 1
    hist = next(p for p in rebuilt if p.get("id") == "hist")
    assert hist["endDate"] == 1780000000000
    newbie = next(p for p in rebuilt if "id" not in p)
    assert newbie["startDate"] == start_ms
...
def test_price_only_writes_run_currency_when_live_differs(db_session):
    run, row = _run_row(db_session, intent="price_only")
    live = _live_ss(price="40.00")
    live["articlePrices"][0]["currencyId"] = "999"
    client = FakeClient(get_map={"/articleSupplySource/id/ss-1": live})
    apply_chunk(
        db_session, run, oid=PLAIN["oid"], actor_name="Dennis", client=client, chunk_index=0
    )
    ss_puts = [c for c in client.put_calls if "/articleSupplySource/" in c["path"]]
    prices = ss_puts[0]["json"]["articlePrices"]
    new_rows = [p for p in prices if "id" not in p]
    assert new_rows
    assert new_rows[0]["currencyId"] == "261"
    db_session.refresh(row)
    assert row.currency_overwritten is True
    assert row.live_currency_id == "999"
    assert row.resolved_currency_id == "261"
