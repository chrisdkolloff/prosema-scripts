...
def test_format_weclapp_price_caps_fraction_digits():
    from app.supply_source_payload import format_weclapp_price

    assert format_weclapp_price(Decimal("49.90")) == "49.9"
    assert format_weclapp_price("48.9") == "48.9"
    assert format_weclapp_price(49.900000000000006) == "49.9"
    long_frac = Decimal("1") / Decimal("3")
    text = format_weclapp_price(long_frac)
...
def test_put_quantizes_copied_live_price_floats():
    body, _ = build_supply_source_put(
        supply_source_id="1",
        version="1",
        article_prices=[{"id": "353344", "price": Decimal("48.9099999")}],
        live_get=LIVE_GET,
    )
    assert body["articlePrices"][0]["price"] == "48.91"
