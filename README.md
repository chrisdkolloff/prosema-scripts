...
`--push` runs `alembic upgrade head` against `PRODUCTION_DATABASE_URL` first (schema before new code), then `scripts/squash-merge-dev-to-main.sh --push --auto-message`: one commit on `main`, reset `dev` to that commit, push `main`, force-with-lease push `dev`. GitHub Actions then deploys the zip to App Service. Azure Postgres does not accept connections from GitHub-hosted runners, so migrations stay in `release.sh`. Azure deploys from `origin/main`.
