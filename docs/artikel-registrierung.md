...
Later releases run `alembic upgrade head` from `./release.sh --push` before
GitHub Actions deploys the zip. Migrations are not run on GitHub-hosted
runners (Azure Postgres firewall).
